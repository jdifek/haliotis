...
const Diving = ({ params }: { params: Promise<{ locale: string }> }) => {
  ...
  const [freediveData, setFreediveData] = useState<any | null>(null);
  ...
  return (
    <main className="-mt-[97px]">
      ...
      <FreediveSitesSection
        regions={
          freediveData?.regions?.entities?.map((entity: any) => ({
            id: entity.id,
            name: entity.name,
        ...
        locations={
          freediveData?.regions?.entities?.flatMap(
            (entity: any) =>
              entity.locations?.map((loc: any) => ({
                ...
                certification: loc.certifications
                  ? loc.certifications.join(", ")
                  : "",
