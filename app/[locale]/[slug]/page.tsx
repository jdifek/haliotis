...
async function getMenuPages(locale: string): Promise<MenuPage[]> {
  const url = `${process.env.NEXT_PUBLIC_API_URL}/configs/menus?lang=${locale}`;
  console.log("[DynamicPage] fetching menu pages:", url);
  ...
  try {
    ...
    console.log("[DynamicPage] menu pages response status:", res.status);

    if (!res.ok) {
      console.log("[DynamicPage] menu pages fetch NOT ok, returning []");
      return [];
    }

    const json = await res.json();

    // Покажет точно, какие ключи реально лежат внутри data
    console.log("[DynamicPage] keys in json.data:", Object.keys(json?.data ?? {}));
    console.log("[DynamicPage] does json.data have 'pages'?", "pages" in (json?.data ?? {}));
    console.log("[DynamicPage] json.data.pages value:", JSON.stringify(json?.data?.pages));
console.log(json, 'jsonjsonjson');

    const pages = json?.pages ?? [];
    console.log(
      "[DynamicPage] menu pages slugs:",
      JSON.stringify(pages.map((p: MenuPage) => p.slug))
    );
    return pages;
  } catch (error) {
    console.error("[DynamicPage] Error fetching menu pages:", error);
    return [];
  }
}

async function getPage(slug: string, locale: string): Promise<PageData> {
  const url = `${process.env.NEXT_PUBLIC_API_URL}/pages/${slug}?lang=${locale}`;
  console.log("[DynamicPage] fetching page:", url);

  const res = await fetch(url, {
    headers: { Accept: "application/json" },
  });

  console.log("[DynamicPage] page response status:", res.status);

  if (!res.ok) {
    const text = await res.text().catch(() => "<no body>");
    console.log("[DynamicPage] page fetch failed body:", text);
    throw new Error(`Failed: ${res.status}`);
  }

  const json = await res.json();
  console.log("[DynamicPage] page json:", JSON.stringify(json));
...
export default async function DynamicPage({
  ...
  const { locale, slug } = await params;
  console.log("[DynamicPage] params:", { locale, slug });

  // Проверка: есть ли такая страница в menuData.pages
  const pages = await getMenuPages(locale);
  const pageExists = pages.some((p) => p.slug === slug);

  console.log("[DynamicPage] looking for slug:", slug, "-> exists:", pageExists);

  if (!pageExists) {
    console.log("[DynamicPage] slug NOT found in menu pages, calling notFound()");
    notFound();
  }

  let data: PageData;
  try {
    data = await getPage(slug, locale);
  } catch (error) {
    console.error("[DynamicPage] getPage threw, calling notFound():", error);
    notFound();
  }

  console.log("[DynamicPage] rendering page with data.title:", data.title);
