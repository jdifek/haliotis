...
// ─── Static payment placeholder data (replace with real API data later) ───────
// TODO: подключить реальные данные (entity/reference/amount/expireDate)
// через query string, API или localStorage — сейчас просто заглушки.
...
// ─── Icons ────────────────────────────────────────────────────────────────────
// Иконка над заголовком — пользователь вставит свою (svg) сам, поэтому здесь
// просто плейсхолдер-обёртка нужного размера/формы под каждый статус.

const IconSlot = ({ children }: { children: React.ReactNode }) => (
  <div className="w-14 h-14 flex items-center justify-center">{children}</div>
);

// Временные заглушки-иконки (замени на свои SVG)
const SuccessIconPlaceholder = () => (
<svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
  <g clip-path="url(#clip0_608_3463)">
    <path d="M60 0H0V60H60V0Z" fill="white" fill-opacity="0.01" />
    <path d="M30 5L36.5667 9.79005L44.6947 9.77458L47.1917 17.5096L53.7764 22.2746L51.25 30L53.7764 37.7254L47.1917 42.4904L44.6947 50.2254L36.5667 50.21L30 55L23.4334 50.21L15.3054 50.2254L12.8084 42.4904L6.22363 37.7254L8.75005 30L6.22363 22.2746L12.8084 17.5096L15.3054 9.77458L23.4334 9.79005L30 5Z" fill="#A0C52E" stroke="black" stroke-width="5" stroke-linecap="round" stroke-linejoin="round" />
    <path d="M21.25 30L27.5 36.25L40 23.75" stroke="white" stroke-width="5" stroke-linecap="round" stroke-linejoin="round" />
  </g>
  <defs>
...
<svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
  <g clip-path="url(#clip0_608_3687)">
...
<svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
  <g clip-path="url(#clip0_608_3923)">
...
// ─── Shared typography ──────────────────────────────────────────────────────────
// font-family: var(--font-family); font-weight: 500; font-size: 48px;
// line-height: 130%; text-align: center; color: #000;
...
// ─── Shared shell ───────────────────────────────────────────────────────────────
// Header/Footer приходят из layout.tsx — здесь только центральный блок страницы,
// повторяющий контейнер из карточки корзины (max-w, paddings, bg, border-radius).
// Отступ 20px на пк между свг / текстами / кнопкой — единый gap на колонке.
...
const MultibancoContent = () => {
  const p = MULTIBANCO_PLACEHOLDER; // TODO: заменить на реальные данные платежа
...
>
  ...
  <div
    className="rounded-[12px] bg-white px-[14px]"
    style={{ background: "#f1f1f1" }}
  >
    <MultibancoRow label="Entity:" value={p.entity} />
    <MultibancoRow label="Reference:" value={p.reference} />
    <MultibancoRow label="Amount:" value={`${p.amount} ${p.currency}`} />
    <MultibancoRow label="Valid until:" value={p.expireDate} />
  </div>
...
export default async function CartStatusPage({ params }: PageProps) {
  ...
  return (
    <StatusShell>
      ...
      {status === "multibanco" && <MultibancoContent />}
    </StatusShell>
  );
}