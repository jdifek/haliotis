            ...
            {options.map((option) => {
              ...
              return (
                <button
                  ...
                  className={`w-full px-3 py-2 text-left text-[15px] hover:bg-[#f5f5f5] cursor-pointer ${
                    isSelected ? "bg-[#f7e4de] text-[#e84814]" : "text-[#111]"
                  }`}
                >
                  {optLabel}
                  {unit && typeof option !== "object" ? ` ${unit}` : ""}
                </button>
              );
            })}
          </div>
      ...
      {legendVisible && (
        <div className="flex items-center justify-center gap-2 pb-3">
          ...
          <span className="text-[12px] text-[#999]">
            {t("unavailable_days", "Unavailable days")}
          </span>
        </div>
      )}
    </div>
...
const EquipmentGrid = ({ equipment, onToggle, onSelectVariation }) => (
  <div className="flex flex-wrap gap-3 mt-2 pt-4 pl-4">
    {equipment.map((item) => (
      <div
        key={item.id}
        className={`relative flex-shrink-0 w-[130px] transition-colors rounded-2xl p-2 ${
      ...
      >
        <button
          ...
          style={{
            ...
            width: 48,
            height: 48,
            background: item.isSelected ? "#e84814" : "#000",
            border: "none",
          }}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[14px] font-semibold text-[#111]">
            {t("participant", "Participant")} {pNum}
          </span>
        </div>
        <ChevronDown
            ...
            <>
              ...
              >
                <span className="text-[13px] font-bold text-[#111]">
                  {t(
                    "additional_equipment_for_participant",
                    "Additional Equipment for Participant"
                  )}{" "}
                  <span className="text-[#e84814]">{pNum}</span>
                </span>
                <ChevronDown
                  className={`text-[#666] transition-transform ${
                    equipExpanded ? "rotate-180" : ""
                  }`}
                />
              </button>
              ...
              {equipExpanded && (
                <EquipmentGrid
                  ...
                  onSelectVariation={(eid, vid) =>
                    onSelectVariation(activityId, shared.id, eid, vid)
                  }
                />
              )}
            </>
            ...
            <div className="border border-[#e84814] rounded-2xl p-3 mt-2 bg-[#fff8f6]">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[13px] font-semibold text-[#e84814]">
                  {t(
                    "dive_certification_required_title",
                    "Dive Certification — required for this activity"
                  )}
                </span>
              </div>
              ...
              <div className="grid md:grid-cols-2 grid-cols-1 gap-2">
                <PlaceholderInput
                  ...
                  value={shared.totalDives || ""}
                  onChange={(v) => onChangeShared(shared.id, "totalDives", v)}
                  type="number"
                />
                <DatePickerField
                  value={shared.lastDiveDate || ""}
                  onChange={(v) => onChangeShared(shared.id, "lastDiveDate", v)}
              ...
              >
                ...
                >
                  {t(
                    "dive_certification_required_alert",
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                {!activity.available && (
                  <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-[#f5f5f5] text-[#999]">
                    {t("unavailable", "Unavailable")}
                  </span>
                )}
              </div>
              <h3 className="text-[14px] font-semibold text-[#111] leading-[140%]">
                {activity.title}
              </h3>
              {activity.subtitle && (
    ...
    <div className="bg-white rounded-2xl border border-[#e4e4e4] p-4 sticky top-4">
      ...
      {activities.map((act) => {
        ...
        const selectedEquipByP = sharedParticipants.map((sp) => ({
          ...
          items: (act.byParticipant[sp.id]?.equipment || []).filter(
            (e) => e.isSelected
          ),
        }));
        ...
        return (
          <div key={act.id} className="mb-4">
            <div className="flex items-start justify-between gap-2 mb-2">
              <p className="text-[13px] text-[#111] leading-[140%] flex-1">
                {act.title}
              </p>
              <span className="text-[16px] font-bold text-[#111] flex-shrink-0">
                {act.currency}
                {act.price}
              </span>
            </div>
            <div className="bg-[#f5f5f5] rounded-2xl px-3 py-2 flex flex-col gap-1">
              <div className="flex justify-between text-[13px] text-[#444]">
                <span>
                  {sharedParticipants.length} {t("adults", "Adults")} ×{" "}
                  {act.currency} {act.price}
                </span>
                <span>
                  {act.currency} {courseTotal.toFixed(2)}
                </span>
              </div>
              {selectedEquipByP.map(
                (p, idx) =>
                  p.items.length > 0 && (
                    <div key={idx}>
                      ...
                      {p.items.map((item) => (
                        <div
                          key={item.id}
                          className="flex justify-between text-[13px] text-[#444] py-0.5"
                        >
      ...
      <div className="flex flex-col gap-2 mb-4">
        ...
        <CheckboxRow
          ...
          onChange={setTerms}
          t={t}
          label={t("terms_conditions_label", "Termos e Condições")}
        />
      </div>
...
{!payment && (
        ...
        <PaymentModal>
          ...
          {submitSuccess && (
            <div className="px-3 py-2 mb-3 rounded-[10px] bg-[#f0fff4] border border-[#4caf50] text-[13px] text-[#2e7d32]">
              {t(
                "booking_success_pay",
                "Booking created! Complete your payment below."
              )}
            </div>
          )}
          {!widgetReady && (
      ...
      <div className="flex items-center justify-center gap-1.5 mt-2">
        ...
        <span className="text-[11px] text-[#999]">
          {t("secure_checkout", "Secure checkout · SSL encrypted")}
        </span>
      </div>
    </div>
  );
...
export default function CartPage() {
      ...
      if (c > prev.length) {
        return [
          ...
          ...Array.from({ length: c - prev.length }, (_, i) =>
            createSharedParticipantWithDefaults(
              prev.length + i + 1,
              measurements
            )
          ),
      ...
      prev.map((a) => {
        ...
        if (c > ids.length) {
          for (let i = ids.length + 1; i <= c; i++) {
            nextByParticipant[i] = createActivityParticipantData(
              a.equipmentTemplate
            );
          }
        } else {
    ...
    return (
      <div className="min-h-screen bg-[#f5f5f5] flex items-center justify-center">
        <p className="text-[15px] text-[#999]">
          {t("loading_cart", "Loading your cart…")}
        </p>
      </div>
    );
  }
  ...
  if (loadError) {
    return (
      <div className="min-h-screen bg-[#f5f5f5] flex items-center justify-center">
        <p className="text-[15px] text-[#e84814]">
          {t("load_cart_error", "Could not load cart:")} {loadError}
        </p>
      </div>
    );
  }
      ...
      >
        <div className="max-w-[1200px] mx-auto px-4 py-8">
          ...
          <p className="text-[13px] text-[#666] mb-6">
            {activities.length} {t("activities", "activities")} ·{" "}
            {participantCount} {t("participants", "participants")}
          </p>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="bg-white rounded-2xl border border-[#e4e4e4] p-4 mb-4 flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[15px] font-semibold text-[#111]">
                    {t("participants_label", "Participants")}{" "}
                    <span className="text-[#e84814]">*</span>
                  </span>
                </div>
                <ParticipantsCounter
              ...
              {activities.length === 0 ? (
                <div className="bg-white rounded-2xl border border-[#e4e4e4] p-12 text-center">
                  ...
                  <p className="text-[14px] text-[#bbb] mt-2">
                    {t(
                      "empty_cart_subtitle",
