...
const addCourseToCart = (id: number | string, location?: string) => {
  try {
    ...
    const cart: Array<{ type: string; id: number | string; location?: string }> =
      raw ? JSON.parse(raw) : [];

    // Не добавлять дубликат
    const exists = cart.some((item) => item.type === "course" && item.id === id);
    if (!exists) {
          ...
          <div className="flex flex-col gap-4 md:gap-6 flex-1">
            ...
            <div className="flex items-center gap-3 md:justify-start justify-between">
              ...
              <ButtonWithIcon
                ...
                label={addedToCart ? t("added_to_cart", "Added!") : t("add_to_cart", "Add to Cart")}
                onClick={handleAddToCart}
                icon={
                  addedToCart ? (
                    // Галочка когда добавлено
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M20 6L9 17L4 12" stroke="#22a35a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  ) : (
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M6 2L3 6V20C3 20.5304 3.21071 21.0391 3.58579 21.4142C3.96086 21.7893 4.46957 22 5 22H19C19.5304 22 20.0391 21.7893 20.4142 21.4142C20.7893 21.0391 21 20.5304 21 20V6L18 2H6Z" stroke="#281D4D" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                      <path d="M3 6H21" stroke="#281D4D" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                      <path d="M16 10C16 11.0609 15.5786 12.0783 14.8284 12.8284C14.0783 13.5786 13.0609 14 12 14C10.9391 14 9.92172 13.5786 9.17157 12.8284C8.42143 12.0783 8 11.0609 8 10" stroke="#281D4D" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  )
                }
              />
