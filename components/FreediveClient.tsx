...
import { DivingCenter, Terms, useMenu } from "@/app/hooks/useMenu";
...
type FreediveClientProps = {
  locale: string;
  slugFromUrl: string | undefined;
  divingCenters: DivingCenter[];
  colorBySlug: Record<string, string>;
  terms: Terms;
  initialData: DivingData | null;
  initialActiveTab: string | null;
};

const FreediveClient = ({
  locale,
  slugFromUrl,
  divingCenters,
  colorBySlug,
  ...
  const [activeTab, setActiveTab] = useState<string | null>(initialActiveTab); // было: null
  const [divingData, setDivingData] = useState<DivingData | null>(initialData); // было: null
...
useEffect(() => {
  const update = () => {
    setColCount(window.innerWidth >= 1670 ? 3 : window.innerWidth >= 700 ? 2 : 1);
  };
  update();
...
const divingDataRef = useRef<DivingData | null>(null);
useEffect(() => { console.log('📌 activeTab changed:', activeTab); }, [activeTab]);
  useEffect(() => { console.log('📌 divingData changed:', divingData?.dive_trips?.length, 'trips'); }, [divingData]);
  useEffect(() => { console.log('📌 dataLoading changed:', dataLoading); }, [dataLoading]);
  useEffect(() => { console.log('📌 expandedCardId changed:', expandedCardId); }, [expandedCardId]);
  useEffect(() => { console.log('📌 expandedDetails changed:', expandedDetails?.length, 'chars'); }, [expandedDetails]);
  useEffect(() => { console.log('📌 detailsLoading changed:', detailsLoading); }, [detailsLoading]);
  useEffect(() => { console.log('📌 colCount changed:', colCount); }, [colCount]);
  useEffect(() => { console.log('📌 divingCenters changed, length:', divingCenters?.length); }, [divingCenters]);
  useEffect(() => { console.log('📌 terms changed'); }, [terms]);
  divingDataRef.current = divingData;
  // Считаем реальное количество колонок из DOM
  ...
  // Авто-раскрытие карточки по slug из URL
useEffect(() => {
    if (!slugFromUrl || !divingData?.dive_trips?.length) return;

    const index = divingData.dive_trips.findIndex(
      (trip) => trip.slug === slugFromUrl
    );

    if (index !== -1) {
      const page = Math.floor(index / CARDS_PER_PAGE) + 1;
      const pageIndex = index % CARDS_PER_PAGE;

      setCurrentPage(page);
      setExpandedCardId(pageIndex);

      setDetailsLoading(true);
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/dive-trips/${slugFromUrl}?lang=${locale}`,
        { headers: { Accept: "application/json" } }
      )
        .then((r) => r.json())
        .then((json) => setExpandedDetails(json.data?.description ?? null))
        .catch(console.error)
        .finally(() => setDetailsLoading(false));

      setTimeout(() => {
        cardRefs.current[pageIndex]?.scrollIntoView({
          behavior: "smooth",
          block: "center",
        });
      }, 400);
...
console.log(divingData, 'divingData');
  ...
  // Строки считаем по реальному colCount из DOM
const rows = useMemo<TripCard[][]>(() => {
    const result: TripCard[][] = [];
    for (let i = 0; i < paginatedCards.length; i += colCount) {
      result.push(paginatedCards.slice(i, i + colCount));
    }
    return result;
  }, [paginatedCards, colCount]);
...
if (slug) {
  window.history.replaceState(null, "", `/${locale}/freedive/${slug}`);
    ...
    if (slug) {
      setDetailsLoading(true);
      fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/dive-trips/${slug}?lang=${locale}`,
        { headers: { Accept: "application/json" } }
      )
        .then((r) => r.json())
        .then((json) => setExpandedDetails(json.data?.description ?? null))
...
/>
  ...
  {/* Mobile center select — как в Courses */}
<div className="md:hidden mb-6 flex flex-col gap-2">
  <button
    onClick={() => setIsTabOpen(!isTabOpen)}
    className="flex items-center cursor-pointer justify-between rounded-[10px] px-3 py-2 h-[42px] bg-white border-1"
  ...
  <div
          ...
          style={{ opacity: dataLoading ? 0.4 : 1, transition: "opacity 0.3s" }}
        >
          {rows.map((row, rowIndex) => {
            const rowStartIndex = rowIndex * colCount;
            return (
<div key={`row-${rowIndex}`}>                <div
...
>
                        <ArticleCard
                          ...
                          link={card.link}
                          details={
                            expandedCardId === globalIndex
                              ? (expandedDetails ?? card.details)
                              : card.details
                          }
                          detailsLoading={
                            expandedCardId === globalIndex
                          ...
                          onToggleExpand={() =>
                            handleToggleExpand(globalIndex, card.slug)
                          }
                          onBookClick={() =>
                  ...
                  >
                    ...
                    <div className="flex flex-col lg:flex-row gap-8">
                      <div
                        ...
                        style={{
                          opacity: detailsLoading ? 0.4 : 0.8,
                      ...
                      >
                        <div
                          dangerouslySetInnerHTML={{
                            __html: expandedDetails ?? expandedCard.details,
                          }}
                        />
                      </div>
