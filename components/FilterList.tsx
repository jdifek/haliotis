    ...
    <div className="flex w-full max-w-[282px] flex-col gap-2">
      {options.map((option) => (
        <button
          ...
          className={`flex items-center cursor-pointer justify-between rounded-lg px-3.5 py-2.5 transition-colors ${
            option.id === selected
              ? bgActive + " " + " h-[38px] bg-[#fadc00]"
              : bgNoactive + " " +  "h-[38px] bg-white hover:bg-gray-50"
          } `}
        >
          <span
            className={`text-start uppercase leading-[120%] text-black ${
              option.id === selected
                ? textActive + " " + "text-[16px] font-bold"
