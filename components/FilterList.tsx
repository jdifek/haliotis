...
export const FilterList: React.FC<FilterListProps> = ({
  ...
  const { colorBySlug } = useMenu(locale);

  const colorKey = activeTabId ?? selected;
  const bg = colorBySlug[colorKey] || "#fadc00";
        ...
        return (
          <button
            ...
            className={`flex z-20 items-center cursor-pointer justify-between rounded-lg px-3.5 py-2.5 transition-colors h-[38px] ${
              !defaultSelectedBg && !isSelected ? "bg-white hover:bg-gray-50" : ""
            }`}
            style={{
              backgroundColor: isSelected ? bg : defaultSelectedBg || undefined,
            }}
          >
            <span
              className={`text-start uppercase leading-[120%] ${
                isSelected ? "text-[16px] font-bold text-white" : "text-[15px] font-normal text-black"
              }`}
            >
              {option.label}
            </span>
