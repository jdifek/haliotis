...
export const Header: React.FC<{ locale: string; logoUrl: string; logoAlt: string }> = ({ locale, logoUrl, logoAlt }) => {
...
<div onClick={handleToMain} className="relative cursor-pointer flex-shrink-0 xl:hidden">
  <Image src={logoUrl} alt={logoAlt} width={173} height={41} className="h-[41px] w-auto" />
</div>

<div onClick={handleToMain} className="relative cursor-pointer flex-shrink-0 hidden xl:block">
  <Image src={logoUrl} alt={logoAlt} width={158} height={25} className="h-[clamp(18px,1.4vw,24px)] w-auto" />
</div>
          {/* Desktop Nav */}
          <nav className="relative hidden rounded-xl bg-black/10 px-[clamp(8px,1vw,16px)] py-[clamp(10px,0.9vw,16.5px)] xl:flex">
            <div className="flex w-full items-center justify-center gap-[clamp(8px,1.4vw,28px)]">
              {navItems.slice(0, -1).map((item) => (
                <div key={item.id} className="relative">
                  <button
                    ref={
                      item.id === "centers" ? centersButtonRef
                        : item.id === "courses" ? coursesButtonRef
                        : undefined
                    }
                    onClick={() => {
                  ...
                  {/* Centers Dropdown */}
                  {item.hasDropdown && openDropdown === item.id && item.id === "centers" && (
                    <div
                      ref={dropdownRef}
                    ...
                    >
                      {centersData.map((center, index) => (
                        <button
                          key={center.id}
                          onClick={() => { setSelectedCenter(center.id); router.push("/centers"); setOpenDropdown(""); }}
            ...
            {openDropdown === "courses" && (
              <div
                ...
                style={{ width: "100%", maxWidth: "100%", background: "rgba(0, 3, 38, 0.5)", backdropFilter: "blur(10px)", boxShadow: "0 4px 4px 0 rgba(0,0,0,0.25)" }}
              >
                <div className="flex flex-wrap justify-center gap-[10px]">
                  {coursesData.map((course) => (
                    <Link key={course.id} href="/courses" className="flex flex-col items-center gap-[5px] cursor-pointer rounded-[20px] border-2 border-transparent px-0 pb-[5px] pt-[10px] transition-all hover:border-white hover:bg-[#111d9e]" style={{ width: "120px", height: "124px" }}>
                      <div className="h-[80px] w-[80px] overflow-hidden rounded-[16px]">
                        <Image src={course.image} alt={course.label} width={80} height={80} className="h-full w-full object-cover" />
                      </div>
                      <span className="text-center text-[15px] font-semibold leading-[160%] text-white" style={{ fontFamily: "var(--font-family)" }}>
                        {course.label}
                      </span>
                    </Link>
                  ))}
                  ...
                  >
                    {languages.filter((l) => l?.prefix).map((lang, index) => (
                      <button
                        ...
                        className={`flex h-[40px] w-full cursor-pointer items-center justify-center text-[15px] font-semibold transition-colors ${selectedLang === lang.prefix.toUpperCase() ? "text-[#e84814]" : "text-white hover:text-[#e84814]"} ${index === 0 ? "rounded-t-[8px]" : index === languages.length - 1 ? "rounded-b-[8px]" : ""}`}
          ...
          <div className="flex h-full flex-col relative">
            ...
            <div className="flex items-center justify-between p-5">
              <div className="relative h-10 w-32">
                <svg width="173" height="41" viewBox="0 0 173 41" fill="none"><path d="M0 29.328V11.1298H3.83949V18.7911H13.4312V11.1298H17.2707V29.328H13.4312V22.6312H3.83949V29.328H0Z" fill="white"/></svg>
              </div>
              <button onClick={() => setIsMenuOpen(false)} className="flex items-center justify-center rounded-lg bg-white p-2">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M7 7L17 17M7 17L17 7" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
              </button>
            </div>
                ...
                <div key={item.id} className="relative">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      if (item.hasDropdown) {
                        setMobileOpenDropdown(mobileOpenDropdown === item.id ? null : item.id);
                      } else {
                        setActiveNav(item.id);
                        setIsMenuOpen(false);
                      }
                    }}
                    className={`flex w-full items-center justify-between uppercase text-[15px] font-normal leading-[120%] ${activeNav === item.id ? "text-[#e84814]" : "text-white"}`}
                  >
                    {item.label}
                    {item.hasDropdown && (
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className={`transition-transform ${mobileOpenDropdown === item.id ? "rotate-180" : ""}`}>
                  ...
                  {item.id === "centers" && mobileOpenDropdown === "centers" && (
                    <div className="absolute rounded-lg -left-3.5 -top-2 z-10 w-[calc(100%_+_28px)] overflow-hidden rounded-t-lg" style={{ background: "#fff" }}>
                      <button onClick={() => setMobileOpenDropdown(null)} className="flex h-[42px] w-full items-center justify-between px-[14px] py-[10px]">
                        <span className="text-[15px] font-normal uppercase leading-[120%] text-black">CENTERS</span>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="rotate-180"><path d="M17.8534 9.85369L12.8537 14.8534C12.8073 14.8999 12.7522 14.9367 12.6915 14.9619C12.6308 14.9871 12.5657 15 12.5 15C12.4343 15 12.3692 14.9871 12.3085 14.9619C12.2478 14.9367 12.1927 14.8999 12.1463 14.8534L7.14663 9.85369C7.07663 9.78377 7.02895 9.69465 7.00963 9.59761C6.9903 9.50058 7.00021 9.39999 7.03808 9.30858C7.07595 9.21718 7.1401 9.13907 7.22239 9.08413C7.30468 9.0292 7.40142 8.99992 7.50036 9H17.4996C17.5986 8.99992 17.6953 9.0292 17.7776 9.08413C17.8599 9.13907 17.924 9.21718 17.9619 9.30858C17.9998 9.39999 18.0097 9.50058 17.9904 9.59761C17.971 9.69465 17.9234 9.78377 17.8534 9.85369Z" fill="black"/></svg>
                      </button>
                      <div className="flex flex-col bg-[#1a1a3e] border-2 border-white rounded-lg">
                        {centersData.map((center, index) => (
                          <button
                            key={center.id}
                            onClick={() => { setSelectedCenter(center.id); router.push("/centers"); setIsMenuOpen(false); setMobileOpenDropdown(null); }}
                            className={`flex h-[44px] items-center cursor-pointer justify-between px-[14px] transition-all hover:bg-[#111d9e] ${selectedCenter === center.id ? "bg-[#111d9e]" : ""} ${index === centersData.length - 1 ? "rounded-b-lg" : ""}`}
                          >
                            <span className="text-[15px] font-semibold uppercase leading-[160%]" style={{ color: center.color }}>{center.label}</span>
                            {selectedCenter === center.id && (
                              <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M9.8537 6.14663L14.8534 11.1463C14.8999 11.1927 14.9367 11.2478 14.9619 11.3085C14.9871 11.3692 15 11.4343 15 11.5C15 11.5657 14.9871 11.6308 14.9619 11.6915C14.9367 11.7522 14.8999 11.8073 14.8534 11.8537L9.8537 16.8534C9.78377 16.9234 9.69465 16.971 9.59762 16.9904C9.50058 17.0097 9.39999 16.9998 9.30859 16.9619C9.21718 16.924 9.13907 16.8599 9.08414 16.7776C9.0292 16.6953 8.99992 16.5986 9 16.4996L9 6.50036C8.99992 6.40142 9.0292 6.30468 9.08414 6.22239C9.13907 6.1401 9.21718 6.07595 9.30859 6.03808C9.39999 6.00021 9.50058 5.99031 9.59761 6.00963C9.69465 6.02895 9.78377 6.07663 9.8537 6.14663Z" fill={center.color}/></svg>
                            )}
                          </button>
                        ))}
                      </div>
                  ...
                  {item.id === "courses" && mobileOpenDropdown === "courses" && (
                    <div className="absolute -left-3.5 -top-2 z-10 w-[calc(100%_+_28px)] overflow-hidden rounded-lg" style={{ background: "#fff", maxHeight: "600px", overflowY: "auto" }}>
                      <button onClick={() => setMobileOpenDropdown(null)} className="flex h-[42px] w-full items-center justify-between px-[14px] py-[10px]">
                        <span className="text-[15px] font-normal uppercase leading-[120%] text-black">COURSES</span>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="rotate-180"><path d="M17.8534 9.85369L12.8537 14.8534C12.8073 14.8999 12.7522 14.9367 12.6915 14.9619C12.6308 14.9871 12.5657 15 12.5 15C12.4343 15 12.3692 14.9871 12.3085 14.9619C12.2478 14.9367 12.1927 14.8999 12.1463 14.8534L7.14663 9.85369C7.07663 9.78377 7.02895 9.69465 7.00963 9.59761C6.9903 9.50058 7.00021 9.39999 7.03808 9.30858C7.07595 9.21718 7.1401 9.13907 7.22239 9.08413C7.30468 9.0292 7.40142 8.99992 7.50036 9H17.4996C17.5986 8.99992 17.6953 9.0292 17.7776 9.08413C17.8599 9.13907 17.924 9.21718 17.9619 9.30858C17.9998 9.39999 18.0097 9.50058 17.9904 9.59761C17.971 9.69465 17.9234 9.78377 17.8534 9.85369Z" fill="black"/></svg>
                      </button>
                      <div className="grid grid-cols-3 border-2 border-white rounded-lg gap-[10px] bg-[#1a1a3e] p-[10px] pb-[20px]">
                        {coursesData.map((course) => (
                          <Link key={course.id} href="/courses" onClick={() => { setMobileOpenDropdown(null); setIsMenuOpen(false); }} className="flex flex-col items-center gap-[5px] rounded-[20px] px-0 pb-[5px] pt-[10px]">
                            <div className="h-[80px] w-[80px] overflow-hidden rounded-[16px]">
                              <Image src={course.image} alt={course.label} width={80} height={80} className="h-full w-full object-cover" />
                            </div>
                            <span className="text-center text-[16px] font-normal leading-[140%] text-white">{course.label}</span>
                          </Link>
                        ))}
                      </div>
                    </div>
            ...
            <div className="mt-auto p-6">
              <div className="flex items-center gap-4">
                <div className="relative">
                  ...
                  {isLangMenuOpen && (
                    <div ref={langRef} className="absolute bottom-full left-0 mb-2 flex flex-col rounded-[10px] border-2 border-white" style={{ width: "75px", background: "rgba(0, 3, 38, 0.5)", backdropFilter: "blur(10px)" }}>
                      {languages.filter((l) => l?.prefix).map((lang, index) => (
                        <button
                          key={lang.prefix}
                          onClick={() => { setSelectedLang(lang.prefix.toUpperCase()); switchLocale(lang.prefix); setIsLangMenuOpen(false); }}
                          className={`flex h-[40px] w-[71px] items-center justify-center text-[15px] font-semibold transition-colors ${selectedLang === lang.prefix.toUpperCase() ? "text-[#e84814]" : "text-white hover:text-[#e84814]"} ${index === 0 ? "rounded-t-[8px]" : index === languages.length - 1 ? "rounded-b-[8px]" : ""}`}
                          style={{ background: selectedLang === lang.prefix.toUpperCase() ? "#111d9e" : "transparent" }}
                        >
                          {lang.prefix.toUpperCase()}
                        </button>
